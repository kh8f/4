...
(function(){
  ...
  // مصادر الدخل الجاهزة — سبب إضافة المبلغ
  const INCOME_SOURCES = [
    { id: "salary",   name: "راتب",       icon: "💼" },
    { id: "earned",   name: "كسبتها",     icon: "💪" },
    { id: "gift",     name: "هدية",       icon: "🎁" },
    { id: "sold",     name: "بعت شي",     icon: "🏷️" },
    { id: "refund",   name: "استرداد",    icon: "↩️" },
    { id: "bonus",    name: "مكافأة",     icon: "🌟" },
    { id: "other",    name: "أخرى",       icon: "➕" },
  ];
  function sourceById(id){ return INCOME_SOURCES.find(s => s.id === id) || INCOME_SOURCES[INCOME_SOURCES.length-1]; }
  ...
  function defaultState(){
    return {
      ...
      recurringTemplates: [] // {id, categoryId, amount, note, dayOfMonth, createdAt}
    };
  }
  ...
  function renderHome(){
    ...
    if(_lastBalance !== null && _lastBalance !== balance){
  ...
  function txItemEl(t){
    ...
    const src = t.type === 'income' && t.source ? sourceById(t.source) : null;
    const subParts = [cat.name];
    if(src) subParts.push(`${src.icon} ${src.name}`);
    subParts.push(formatDateShort(t.date));
    if(t.recurring) subParts.push('🔁');
    const div = document.createElement('div');
    div.className = 'tx-item';
    div.innerHTML = `
      <div class="tx-icon" style="background:${cat.color}22; color:${cat.color}">${src ? src.icon : cat.icon}</div>
      <div class="tx-mid">
        <div class="tx-title">${t.note ? escapeHtml(t.note) : (src ? src.name : cat.name)}</div>
  ...
  // ===================== تذكير الاشتراكات المتكررة =====================
  function renderRecurringBanner(){
    const banner = document.getElementById('recurringBanner');
    const curMonth = new Date().toISOString().slice(0,7);
    const today = new Date();
    const pending = state.recurringTemplates.find(tpl => {
      const already = state.transactions.some(t => t.templateId === tpl.id && monthKey(t.date) === curMonth);
      return !already && today.getDate() >= tpl.dayOfMonth;
    });
    if(!pending){ banner.classList.add('hidden'); banner.innerHTML=''; return; }
    const cat = catById(pending.categoryId);
    banner.classList.remove('hidden');
    banner.innerHTML = `
      <span class="rb-icon">🔁</span>
      <div class="rb-mid">
        <div class="rb-title">تذكير: ${escapeHtml(pending.note || cat.name)}</div>
        <div class="rb-sub">اشتراك شهري متكرر · ${fmt(pending.amount)} ${state.currency}</div>
      </div>
      <button class="rb-btn" id="btnLogRecurring">تسجيل الآن</button>
    `;
    document.getElementById('btnLogRecurring').addEventListener('click', () => logRecurringNow(pending));
  }

  function logRecurringNow(tpl){
    state.transactions.push({
      id: uid(), type: 'expense', amount: tpl.amount, categoryId: tpl.categoryId,
      note: tpl.note, date: todayISO(), createdAt: Date.now(), templateId: tpl.id, recurring: true
    });
    saveState();
    renderHome();
    renderTransactionsScreen();
    showToast('تم تسجيل الاشتراك');
  }
  ...
  function renderTransactionsScreen(){
    ...
    if(txSearchQuery){
      list = list.filter(t => {
        const cat = catById(t.categoryId);
        const src = t.source ? sourceById(t.source) : null;
        const haystack = [t.note, cat.name, src ? src.name : ''].join(' ').toLowerCase();
        return haystack.includes(txSearchQuery);
      });
    }
  ...
  function setTxType(type){
    ...
    document.getElementById('sourceField').classList.toggle('hidden', type!=='income');
    document.getElementById('recurringField').classList.toggle('hidden', type!=='expense');
  }
  ...
  function renderSourcePicker(){
    const wrap = document.getElementById('sourcePicker');
    wrap.innerHTML = '';
    INCOME_SOURCES.forEach(src => {
      const pill = document.createElement('button');
      pill.className = 'source-pill' + (selectedSourceId===src.id ? ' active' : '');
      pill.type = 'button';
      pill.innerHTML = `${src.icon} ${src.name}`;
      pill.addEventListener('click', () => { selectedSourceId = src.id; renderSourcePicker(); });
      wrap.appendChild(pill);
    });
    if(!selectedSourceId){ selectedSourceId = INCOME_SOURCES[0].id; renderSourcePicker(); }
  }
  ...
  document.getElementById('btnSaveTx').addEventListener('click', () => {
    ...
    const isRecurring = currentTxType === 'expense' && document.getElementById('txRecurring').checked;
    ...
    if(currentTxType === 'income'){
      tx.source = selectedSourceId || 'other';
    }
    if(isRecurring){
      const dayOfMonth = new Date(date).getDate();
      let tpl = state.recurringTemplates.find(t => t.categoryId===selectedCategoryId && (t.note||'').toLowerCase()===note.toLowerCase());
      if(!tpl){
        tpl = { id: uid(), categoryId: selectedCategoryId, amount, note, dayOfMonth, createdAt: Date.now() };
        state.recurringTemplates.push(tpl);
      } else {
        tpl.amount = amount; tpl.dayOfMonth = dayOfMonth;
      }
      tx.templateId = tpl.id;
      tx.recurring = true;
    }
  ...
  document.getElementById('btnConfirmAddToGoal').addEventListener('click', () => {
    ...
    if(item){
      const wasDone = item.saved >= item.target;
      item.saved += amount;
      if(!wasDone && item.saved >= item.target && item.target > 0) justCompleted = true;
    }
    saveState();
    ...
    if(justCompleted){
      launchConfetti();
      showToast('🎉 وصلت لهدفك! مبروك');
    } else {
      showToast('تمت الإضافة للهدف');
    }
  });

  // احتفال قصاصات ورقية عند إكمال هدف
  function launchConfetti(){
    const colors = ['#D4AF37','#E8C766','#3C8C63','#6FB89A','#E4572E','#FBF0D2'];
  ...
  function renderStatsScreen(){
    ...
    // بطاقات نظرة سريعة + مصادر الدخل
    drawInsightCards(totalsByCat, totalExpense, d);
    drawIncomeSources(income);
  }

  function drawInsightCards(totalsByCat, totalExpense, monthDate){
    const wrap = document.getElementById('insightGrid');
    wrap.innerHTML = '';

    let topCat = null, topAmt = 0;
    Object.entries(totalsByCat).forEach(([id, amt]) => { if(amt > topAmt){ topAmt = amt; topCat = id; } });
    ...
    const dailyAvg = totalExpense / Math.max(1, elapsedDays);

    const cards = [
      {
        label: 'أكثر تصنيف صرف',
        value: topCat ? `${catById(topCat).icon} ${catById(topCat).name}` : '—'
      },
      {
        label: 'متوسط الصرف اليومي',
        value: `${fmt(dailyAvg)} <small>${state.currency}</small>`
      }
    ];
    cards.forEach(c => {
      const div = document.createElement('div');
      div.className = 'insight-card';
      div.innerHTML = `<span class="ic-label">${c.label}</span><span class="ic-value">${c.value}</span>`;
      wrap.appendChild(div);
    });
  }

  function drawIncomeSources(income){
    const wrap = document.getElementById('incomeSourcesList');
    wrap.innerHTML = '';
    const totalsBySource = {};
    income.forEach(t => {
      const sid = t.source || 'other';
      totalsBySource[sid] = (totalsBySource[sid]||0) + t.amount;
    });
    const entries = Object.entries(totalsBySource).sort((a,b)=>b[1]-a[1]);
    document.getElementById('incomeSourcesEmpty').classList.toggle('hidden', entries.length !== 0);
    entries.forEach(([sid, amt]) => {
      const src = sourceById(sid);
      const row = document.createElement('div');
      row.className = 'income-source-row';
      row.innerHTML = `
        <div class="isr-icon">${src.icon}</div>
        <div class="isr-name">${src.name}</div>
        <div class="isr-amt">+ ${fmt(amt)} ${state.currency}</div>
      `;
  ...
  function setTheme(theme){
    ...
    setTheme._t = setTimeout(()=>document.body.classList.remove('theme-transition'), 500);
  }
  function applyTheme(){
  ...
  // ===================== تأثير التموّج (ripple) عند الضغط ===================
  document.addEventListener('click', (e) => {
    const el = e.target.closest('button, .qa-btn, .type-btn, .chip, .cat-pill, .source-pill, .tab-mini, .nav-btn, .nav-fab, .primary-btn, .text-btn, .icon-btn, .link-btn');
    if(!el) return;
